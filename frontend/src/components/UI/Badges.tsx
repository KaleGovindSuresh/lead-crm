...
export const SourceBadge = ({ source }: { source: LeadSource }) => {
  const cls: Record<LeadSource, string> = {
    ...
    referral: 'badge bg-violet-500/20 text-violet-300 border border-violet-500/30',
...
export const RoleBadge = ({ role }: { role: string }) => {
  ...
  return <span className={cls[role] ?? 'badge bg-slate-500/20 text-slate-300'}>{capitalize(role)}</span>
}